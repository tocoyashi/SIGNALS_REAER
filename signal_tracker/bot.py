...
def parse_signal(text):
    ...
    Parse a signal message and extract:
      ...
      - symbol (e.g. BTC/USDT)
      - entry price
      - TP1, TP2, TP3 (optional)
      - SL (stop loss)

    Supports flexible formats like:
      LONG BTC/USDT
      Entry: 65000
      TP1: 66000 | TP2: 68000 | TP3: 70000
      SL: 63000
    """
    data = {}
    ...
    # Symbol: BTC/USDT, ETH/USDT, XAU/USDT, etc.
    ...
    # Entry Price
    entry_match = re.search(r'[Ee]ntry\s*[:\-]?\s*([\d,]+\.?\d*)', text)
    if entry_match:
        data['entry'] = float(entry_match.group(1).replace(',', ''))
    else:
        return None

    # TP levels (support "TP 1:", "TP1:", "TP 1", "TP1" etc.)
    for i in range(1, 4):
        key = f'tp{i}'
    ...
    # SL / Stop Loss
    sl = re.search(r'(?:SL|Stop\s*Loss)\s*[:\-]?\s*([\d,]+\.?\d*)', text, re.IGNORECASE)
    if sl:
        data['sl'] = float(sl.group(1).replace(',', ''))

    # Must have at least one TP and SL
    has_tp = 'tp1' in data or 'tp2' in data or 'tp3' in data
    has_sl = 'sl' in data
